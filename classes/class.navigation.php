<?
	class navigation{
		// This function draws all blocks and links the user has permission to access and returns the string
		function blocks(){
			global $CONN, $DEVIL_CMS_INI;
			$navi = array();
			$sql = "SELECT * FROM ".TAB_BLOCKS." ORDER BY ".COL_BLOCKS_WEIGHT;
		...
		// This function draws all links the user has permission to access and returns the string
		function links($blockID){
			global $CONN, $DEVIL_CMS_INI;
			$naviLinks = array();
			$sql = "SELECT * FROM ".TAB_NAVI." WHERE ".COL_NAVI_BLOCK." = '".sql::escape($blockID)."' ORDER BY ".COL_NAVI_WEIGHT;
		...
		function getBlockGroups($id){
			global $CONN;
			$groups = array();
			$sqlStr = "SELECT ".COL_NAVI_CID." FROM ".TAB_NAVI." WHERE ".COL_NAVI_BLOCK." = '".sql::escape($id)."'";
			$navis = mysql_query($sqlStr, $CONN) or die(mysql_error());
		...
		function updateNaviLinkSorting($naviID, $blockID, $weight){
			...
			$sqlStr = "UPDATE 
												".TAB_NAVI." 
								 SET 
												".COL_NAVI_WEIGHT."='".sql::escape($weight)."',
												".COL_NAVI_BLOCK."='".sql::escape($blockID)."'
								 WHERE 
												".COL_NAVI_ID."='".sql::escape($naviID)."'";
		...
		function updateNaviBlockSorting($blockID, $position, $weight){
			...
			$sqlStr = "UPDATE 
												".TAB_BLOCKS." 
								 SET 
												".COL_BLOCKS_WEIGHT."	='".sql::escape($weight)."',
												".COL_BLOCKS_POS."='".sql::escape($position)."'
								 WHERE 
												".COL_BLOCKS_ID."='".sql::escape($blockID)."'";
			mysql_query($sqlStr, $CONN) or die(mysql_error());
		}
		...
		function deleteBlock($blockID){
			global $CONN;
			$sqlStr = "DELETE FROM ".TAB_BLOCKS." WHERE ".COL_BLOCKS_ID." = '".sql::escape($blockID)."'";
			mysql_query($sqlStr, $CONN)or die(mysql_error());
			
			$sqlStr = "DELETE FROM ".TAB_NAVI." WHERE ".COL_NAVI_BLOCK." = '".sql::escape($blockID)."'";
			mysql_query($sqlStr, $CONN)or die(mysql_error());
		}
		
		function updateBlock($blockID, $blockLabels, $blockDesc){
			global $CONN;
			$sqlStr = "UPDATE ".TAB_BLOCKS." SET 
																						".COL_BLOCKS_DESC."='".sql::escape($blockDesc)."',
		...
		function updateLink($linkID, $labels, $desc, $cid){
			global $CONN;
			$sqlStr = "UPDATE ".TAB_NAVI." SET 
																						".COL_NAVI_CID."='".sql::escape($cid)."',
																						".COL_NAVI_DESC."='".sql::escape($desc)."',
																						".COL_NAVI_LABELS."='".sql::escape(serialize($labels))."',
																						".COL_NAVI_LAST_CHANGE."='".sql::escape(time())."'
																		 WHERE 
																		 				".COL_NAVI_ID."='".sql::escape($linkID)."'";
			mysql_query($sqlStr, $CONN) or die(mysql_error());
		}
		
		function deleteLink($linkID){
			global $CONN;
			$sqlStr = "DELETE FROM ".TAB_NAVI." WHERE ".COL_NAVI_ID." = '".sql::escape($linkID)."'";
			mysql_query($sqlStr, $CONN)or die(mysql_error());
		}
	}
?>
